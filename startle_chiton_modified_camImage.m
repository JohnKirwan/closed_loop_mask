...
threshold = 165;
valve_proportion = 0.6;    % amount of animal area given by the valve
girdle_proportion = 0.25 ; % amount of animal area given by the girdle
im_reps = 20 ;             % number of images to average
num_repeats = 5; % number of repeats for each condition
...
%for i = randz
    ...
    elseif randz(i) == 3
        ...
        filename = strcat('valve_', strrep(datestr(datetime('now')),':',''),'.png') ;
    ...
    elseif randz(i) == 4
        ...
        cam_img = uint8(sumImage/im_reps);
        %filename = strcat('girdle.png', strrep(datestr(datetime('now')),':','')) ;
        imwrite(cam_img,'girdle.png');
        info(i,3) = 'Girdle';
    else
